...
public class SmtpServerIT {
	...
	static final Email email = EmailBuilder.startingBlank().from("foo@bar.com").to("bar@foo.com")
			.withSubject("TestMail").withPlainText("This is a test mail").buildEmail();
	...
	public void test_auth() throws Exception {
		...
		Mailer mailer = getMailer().withTransportStrategy(TransportStrategy.SMTP).withSMTPServerUsername("user")
				.withSMTPServerPassword("password").buildMailer();
		mailer.sendMail(email);
	...
	public void test_auth_bad() throws Exception {
		...
		try {
			Mailer mailer2 = getMailer().withTransportStrategy(TransportStrategy.SMTP).withSMTPServerUsername("user")
					.withSMTPServerPassword("wordpass").buildMailer();
	...
	public void test_ssl() throws Exception {
		...
		getMailer().withTransportStrategy(TransportStrategy.SMTPS)
				.withProperty("mail.smtps.ssl.checkserveridentity", "false").buildMailer().sendMail(email);
	...
	public void test_starttls() throws Exception {
		...
		getMailer().withTransportStrategy(TransportStrategy.SMTP_TLS)
				.withProperty("mail.smtp.ssl.checkserveridentity", "false").buildMailer().sendMail(email);
